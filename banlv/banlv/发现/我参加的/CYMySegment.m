...
- (void)setSelectIdx:(NSInteger)selectIdx{
    ...
    self.underLayer.frame = CGRectMake(self.selectIdx * self.perWidth, self.height - 5, self.perWidth, 3);
...
//    NSLog(@"hello == %zi",self.selectIdx);
    
    self.changeCount(self.selectIdx);
    
//    //tableView向右滑动
//    if (self.lastSelectIdx < self.selectIdx) {
//        self.changeDirection(CYMySegmentDirectionLeft);
//    }
//    //tableView向左滑动
//    else if (self.lastSelectIdx > self.selectIdx) {
//        self.changeDirection(CYMySegmentDirectionRight);
//    }
...
- (void)drawRect:(CGRect)rect {
    ...
    [self.items enumerateObjectsUsingBlock:^(NSString *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSDictionary *attributes = @{NSFontAttributeName:[UIFont systemFontOfSize:22],NSForegroundColorAttributeName:idx == self.selectIdx?self.perColor:self.defaultPerColor,NSParagraphStyleAttributeName:style};
